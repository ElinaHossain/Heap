class Heap<Key extends Comparable<Key>, Value> implements HeapInterface<Key, Value> {
   ...
   public void heapSort() { 
    for (int i = 0; i < currentPosition; i++) { 
        HeapItem tmp = heap[0]; // max element 
        heap[0] = heap[currentPosition - i]; // bring last element to the root 
        heap[currentPosition - i] = tmp; // put max at the last of unsorted part 
        fixDown(0, currentPosition - i - 1); 
         } 
   }

    public void heapify(HeapItem[] data) {
    for (int i = 0; i < data.length; i++) {
        Key k = (Key) data[i].key;
        Value v = (Value) data[i].val;
        insert(k, v);
        }
   }
